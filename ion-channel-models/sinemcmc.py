...
import models_forward.thermo_ind_noise_likelihood as tiLogLikelihood
import models_forward.armax_ode_tsa_likelihood as dsLogLikelihood
...
from sklearn.externals import joblib as LoaderJob
...
parser.add_argument('--discrepancy', type=bool, default=True, metavar='N',
                    help='Run with discrepancy noise')
parser.add_argument('--P', type=int, default=2, metavar='N',
                    help='AR order of ARMA')
parser.add_argument('--Q', type=int, default=1, metavar='N',
                    help='MA order of ARMA')
...
parser.add_argument('--niter', type=int, default=500, metavar='N',
                    help='number of mcmc iterations')
parser.add_argument('--burnin', type=int, default=400, metavar='N',
                    help='number of burn-in samples')
parser.add_argument('--ntemps', type=int, default=8, metavar='N',
                    help='number of temperatures in geomteric ladder')
parser.add_argument('--ppc_samples', type=int, default=250, metavar='N',
                    help='number of burn-in samples')
...
"""
print("loading armax model: ")
pickle_tgt = "./arimax/arima.pkl"
armax_result = LoaderJob.load(pickle_tgt)
"""
cell = args.cell
...
if args.discrepancy:
    ...
    #
    # Fit an ARMAX model based on hERG CMA-ES params
    #
    print('Fitting an ARMAX (', str(args.P),',',str(args.Q),') model')
    cmaes_params = np.loadtxt(cmaes_result_files + model_name +
...
if do_thermo:
    ...
    def mcmc_runner(temps, n_chains = nchains):
        ...
        if args.discrepancy:
            ...
            timer.sleep(1.5)  
            tempered_log_likelihood = dsLogLikelihood.DiscrepancyLogLikelihood(problem, armax_result, temperature=temps)   
            print('Experimental Warning: Running in discrepancy and thermodynamic mode')
            timer.sleep(2.5)      
        else:
            print('Running in iid noise mode')
            timer.sleep(2.5)             
            tempered_log_likelihood = tiLogLikelihood.ThermoLogLikelihood(problem, sigma_noise, temps)
    ...
    if args.discrepancy:
        ...
        print('Initial values of chain from iid noise model')   
        timer.sleep(1.5)  
        log_likelihood = dsLogLikelihood.DiscrepancyLogLikelihood(problem, armax_result, temperature=None)   
